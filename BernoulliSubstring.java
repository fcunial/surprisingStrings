...
public class BernoulliSubstring extends BorderSubstring {
	...
	private double[] out;  // Stores the output of $getScores$ and $getExpectationAndVariance$
...
*/
	...
	protected void popAppendix(Stream stack, Substring[] cache) {
		if (isLeftExtensionOfMaximalRepeat) stack.pop(log2alphabetLength+64*2*nLeft);
		if (leftContext>1) stack.pop(64*2);
	...
	protected void visited(Stream stack, RigidStream characterStack, SimpleStream pointerStack, Substring[] cache, Substring[] leftExtensions) {
		...
		if (isLeftExtensionOfMaximalRepeat) {
			...
			for (i=1; i<alphabetLength; i++) {  // Disregarding $#$
				...
				getExpectationAndVariance(length+1,barPPrime,barPPrime*barPPrime,fPrime,freqPrime,lbPrime,out);
				getScores(freqPrime,out[0],out[1],barPPrime,out);
			}
		}
	}
